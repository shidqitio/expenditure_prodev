...
const sbmTranspor = async (kodeAsal, kodeTujuan, transpor) => {
  try {
    ...
    if (kodeAsal === kodeTujuan) {
      ...
      return response;
    } else if (kodeAsal.substring(0, 5) === kodeTujuan.substring(0, 5)) {
...
const filterSchema = async (req, res, next) => {
  ...
  return await refPerjalananPetugas
    .findAll({
      attributes: [
        "kode_perjalanan",
        "kode_surat_tugas",
      ...
      include: [
        ...
        },
        {
          ...
          as: "petugasPerjadin",
          include: [{ model: KomponenPerjadin, as: "komponenPetugas" }],
        },
      ],
      where: {
      ...
      await Promise.all(
        result.map(async (perjalanan) => {
          ...
          for (let i = 0; i < petugas.length; i++) {
            ...
            let arrayMerge = [];
            const transpor = await sbmTranspor(
              perjalanan.kode_tempat_asal,
              perjalanan.kode_tempat_tujuan,
              perjalanan.transpor
            )
              .then((res) => {
                const dataObj = res.map((trans) => {
                  ...
                  return {
                    ...
                    biaya_satuan:
                      perjalanan.keterangan == "SATU_ARAH"
                        ? trans.biaya_sejalan
                        : trans.biaya_pp,
                    jumlah_satuan: 1,
                    total:
            ...
            const penginapan = await sbmPenginapan(
              element.eselon,
              element.gol,
              perjalanan.kode_tempat_tujuan
            )
            ...
            const uangharian = await sbmUangHarian(
              perjalanan.kode_tempat_tujuan,
              params.skema
            )
              .then((uangharian) => {
          ...
          const petugasPerjalanan = arrayPetugas.filter(
            (data) => data.kode_perjalanan == perjalanan.kode_perjalanan
          );
