...
const perjadinDraft = (req, res, next) => {
  const data = req.body;
    ...
    .then(async (t) => {
      return await SuratTugasPerjadin.create(payloadHeader, {
        ...
        .then(async (result) => {
          ...
          await KomponenPerjadin.bulkCreate(payloadPerjalanan, {
            transaction: t,
          })
            .then(async (result) => {
              if (!result) {
              ...
              await PetugasPerjadin.bulkCreate(payloadPetugas, {
                transaction: t,
              })
                .then((response) => {
                  if (!result) {
